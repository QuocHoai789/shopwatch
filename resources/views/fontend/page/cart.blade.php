                                    ...
                                    @foreach ($carts_user as $cart)
                                        <tr>
                                            ...
                                            <td class="price">
                                                ...
                                                <br>
                                                <span class="price-cart">{{ number_format($cart->products->price) }}</span>

                                            </td>
                                            <td>
                                                <form action="" method="get">
                                                    <div class="form-group" id="table-cart" id="">
                                                        ...
                                                        <button type="submit" class="btn btn-primary update"
                                                            value="{{ $cart->products->id }}" style="z-index: 1">Cập
                                                            nhật</button>
                                                    </div>
                                                </form>
                                            </td>
                                            <td>
                                                <a href=""><button type="button" class="btn btn-danger delete-cart"
                                                        style="z-index: 1" id="{{ $cart->products->id }}">Xóa</button></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
                                    ...
                                    @foreach ($carts as $key => $cart)
                                        <tr>
                                            ...
                                            <td><img src="{{ asset($cart['image']) }}" alt="" class="cart_img"></td>
                                            <td>{{ $cart['name'] }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <form action="" method="get">
                                                    <div class="form-group" id="table-cart" id="">
                                                        <input type="hidden" name="products_id" value="{{ $key }}">
                                                        <input type="number" class="form-control quantily" min=1
                                                            value="{{ $cart['quantily'] }}" name="quantily"
                                                            id="num{{ $key }}">
                                                        <button type="submit" class="btn btn-primary update"
                                                            value="{{ $key }}" style="z-index: 1">Cập
                                                            nhật</button>
                                                    </div>
                                                </form>
                                            </td>
                                            <td>
                                                <a href=""><button type="button" class="btn btn-danger delete-cart"
                                                        id="{{ $key }}" style="z-index: 1">Xóa</button></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @endif
                            ...
                            <div class="address">
                                ...
                                <div class="input-group">
                                    ...
                                    <input id="msg" type="text" class="form-control" name="phone_number"
                                        placeholder="Số điện thoại" value="<?php if (Auth::check()) {
                                            echo Auth::user()->phone_number;
                                        } ?>" required>
                                </div>
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-file-text"></i></span>
                                    <textarea name="note" class="note-cart" placeholder="Ghi chú"
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('.delete-cart').on('click', function(e) {
                e.preventDefault();
                var id = $(this).attr('id');
                url = 'http://localhost:8080/shopwatch/ajax/delete-cart/' + id;
                $.get(
                    url,
                    function() {
